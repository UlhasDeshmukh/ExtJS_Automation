...
public class FillFormTest {
	...
	public void verifyCheckoutForm() {
		...
		/*
		 * Input boxes will have dynamic ids but,
		 * has unique name attribute so 
		...
		driver.findElement(By.name("mailingStreet")).sendKeys(
				"Street Test Address");

		driver.findElement(By.name("mailingCity")).sendKeys("NewYork");

		/*
		 *  This state drop down is not normal dropdown hence,
		 *  first clicked on textbox of dropdown to get focus then,
		 *  clicked on down arrow using css class names which will show
		 *  list of state name
		...
		driver.findElement(
				By.xpath("//div[@class='x-boundlist-list-ct x-unselectable']/ul/li[contains(.,'Arizona')]"))
				.click();
		...
		driver.findElement(
				By.xpath("//div[@class='x-boundlist-list-ct x-unselectable']/ul/li[contains(.,'May')]"))
				.click();
